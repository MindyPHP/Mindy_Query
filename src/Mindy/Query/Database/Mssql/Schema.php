...
class Schema extends \Mindy\Query\Schema\Schema
{
    ...
    {
        $this->getConnection()->createCommand("SAVE TRANSACTION $name")->execute();
    }
    ...
    {
        $this->getConnection()->createCommand("ROLLBACK TRANSACTION $name")->execute();
    }
...
SQL;
        $table->primaryKey = $this->getConnection()
...
SQL;
        $rows = $this->getConnection()->createCommand($sql, [':tableName' => $table->name])->queryAll();
        $table->foreignKeys = [];
        foreach ($rows as $row) {
...
SQL;
        return $this->getConnection()->createCommand($sql, [':schema' => $schema])->queryColumn();
    }
}
