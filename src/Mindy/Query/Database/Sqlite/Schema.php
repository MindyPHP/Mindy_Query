...
class Schema extends \Mindy\Query\Schema\Schema
{
    ...
    protected function findColumns($table)
    {
        ...
        $columns = $this->getConnection()->createCommand($sql)->queryAll();
        if (empty($columns)) {
            return false;
        }
    ...
    protected function findConstraints($table)
    {
        ...
        $keys = $this->getConnection()->createCommand($sql)->queryAll();
        foreach ($keys as $key) {
            $id = (int)$key['id'];
    ...
    public function findUniqueIndexes($table)
    {
        ...
        $indexes = $this->getConnection()->createCommand($sql)->queryAll();
        $uniqueIndexes = [];
        foreach ($indexes as $index) {
            $indexName = $index['name'];
            $indexInfo = $this->getConnection()->createCommand("PRAGMA index_info(" . $this->quoteValue($index['name']) . ")")->queryAll();
            if ($index['unique']) {
    ...
    public function setTransactionIsolationLevel($level)
    {
        switch ($level) {
            case Transaction::SERIALIZABLE:
                $this->getConnection()->createCommand("PRAGMA read_uncommitted = False;")->execute();
                break;
            case Transaction::READ_UNCOMMITTED:
                $this->getConnection()->createCommand("PRAGMA read_uncommitted = True;")->execute();
                break;
            default:
