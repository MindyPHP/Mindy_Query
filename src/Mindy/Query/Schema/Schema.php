...
abstract class Schema
{
    ...
    {
        /* @var $cache Cache */
        if (is_string($this->connection->schemaCache) && class_exists('\Mindy\Base\Mindy')) {
            $cache = \Mindy\Base\Mindy::app()->getComponent($this->connection->schemaCache, false);
        } else {
            $cache = $this->connection->schemaCache;
        }
        if ($this->connection->enableSchemaCache && $cache instanceof Cache) {
    ...
    public function getLastInsertID($sequenceName = '')
    {
        if ($this->connection->isActive) {
            return $this->connection->pdo->lastInsertId($sequenceName === '' ? null : $sequenceName);
        } else {
