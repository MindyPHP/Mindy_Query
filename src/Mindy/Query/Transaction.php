...
class Transaction
{
     ...
     * @throws InvalidArgumentException if [[connection]] is null
     */
    public function begin()
    {
        if (!$this->_active) {
            ...
            // TODO \Yii::trace('Starting transaction', __METHOD__);
            $this->db->open();
    ...
    public function commit()
    {
        if ($this->_active && $this->db && $this->db->isActive) {
            // TODO \Yii::trace('Committing transaction', __METHOD__);
            $this->db->pdo->commit();
            $this->_active = false;
        } else {
    ...
    {
        if ($this->_active && $this->db && $this->db->isActive) {
            // TODO \Yii::trace('Rolling back transaction', __METHOD__);
